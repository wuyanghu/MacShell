...
- (void)readFileAsync:(NSString *)fileName complete:(void (^)(NSString *))complete
{
    dispatch_async(_queue, ^{
        NSString *dirDoc = [FileCacheManager getDoucumentPath];
...
- (void)clearFileCache:(NSArray *)fileNameArr{
    dispatch_sync(_queue, ^{
        NSString *documentsDirectory = [FileCacheManager getDoucumentPath];
...
+ (NSString *)getDoucumentPath{
    ...
    NSString *dirFile = [dirDoc stringByAppendingPathComponent:@"CopyBlockTool"];
    return dirFile;
}
